...
public class InvitationAcceptedEventHandler : IDomainEventHandler<InvitationAccepted>
{
    ...
    public async Task Handle(DomainEvent<InvitationAccepted> notification, CancellationToken cancellationToken)
    {
        ...
        await _panelIntegrationEventService.PublishThroughEventBusAsync(
            new InvitationAcceptedIntegrationEvent(meetingLink, @event.OrganizerId, @event.RecipientName), cancellationToken);
    }
}